...
def get_gsea_results_dir() -> Path:
    """
    Get the GSEA results directory.
    Tries multiple locations:
    1. data/gsea_results/ subdirectory (for deployed apps)
    2. ../analysis_results/deseq2_results/gsea_results/ (for local development)
    """
    current_dir = Path(__file__).resolve().parent
    
    # Try data/ subdirectory first (for deployment)
    data_dir = current_dir / "data" / "gsea_results"
    if data_dir.exists():
        return data_dir
    
    # Fall back to parent directory structure (for local development)
    project_root = get_project_root()
    results_dir = project_root / "analysis_results" / "deseq2_results" / "gsea_results"
    return results_dir


def discover_gsea_files() -> List[Tuple[str, str, str, str]]:
    """
    Discover all GSEA TSV files in the gsea_results directory.
    
    Returns:
        List of tuples: (file_path, comparison_name, database_type, display_name)
        comparison_name: Base comparison name (e.g., "KO_Treatment_vs_Sham_d21")
        database_type: Database type (e.g., "GO_BP", "GO_CC", "GO_MF", "KEGG")
        display_name: Human-readable name for the comparison + database
    """
    gsea_dir = get_gsea_results_dir()
    files = []
    
    if not gsea_dir.exists():
        return files
    
    def clean_comparison_name(name: str) -> str:
        """Clean comparison name for display."""
        # Remove _GSEA_* suffix
        name = name.replace("_GSEA_GO_BP", "").replace("_GSEA_GO_CC", "")
        name = name.replace("_GSEA_GO_MF", "").replace("_GSEA_KEGG", "")
    ...
    # Scan for TSV files
    for file_path in sorted(gsea_dir.glob("*.tsv")):
        name = file_path.stem
        
        # Extract database type
        if "_GSEA_GO_BP" in name:
            db_type = "GO_BP"
            comparison = name.replace("_GSEA_GO_BP", "")
        elif "_GSEA_GO_CC" in name:
            db_type = "GO_CC"
            comparison = name.replace("_GSEA_GO_CC", "")
        elif "_GSEA_GO_MF" in name:
            db_type = "GO_MF"
            comparison = name.replace("_GSEA_GO_MF", "")
        elif "_GSEA_KEGG" in name:
            db_type = "KEGG"
            comparison = name.replace("_GSEA_KEGG", "")
        else:
            continue  # Skip files that don't match pattern
        
        comparison_display = clean_comparison_name(comparison)
        display_name = f"{comparison_display} ({db_type})"
        
        files.append((str(file_path), comparison, db_type, display_name))
    
    return files


def load_gsea_file(file_path: str, use_cache: bool = True) -> pd.DataFrame:
    """
    Load a GSEA TSV file.
    
    Args:
        file_path: Path to the GSEA TSV file
    ...
    try:
        ...
        required_cols = ['pathway', 'pval', 'padj', 'NES']
        missing_cols = [col for col in required_cols if col not in df.columns]
        if missing_cols:
            raise ValueError(f"Missing required columns: {missing_cols}")
        ...
        raise ValueError(f"Error loading GSEA file {file_path}: {str(e)}")
